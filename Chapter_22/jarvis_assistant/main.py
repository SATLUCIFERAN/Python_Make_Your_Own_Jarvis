# CODE LOCATION: jarvis-assistant/main.py (OPTIMIZED FOR FLOW CONTROL)
...
import time
import traceback
from skills.pdf import pdf_parse
...
def initialize_tts_global():
    ...
    if SYSTEM_OS == "Windows":
        try:
            ...
            # FIX 1: Set Windows TTS rate to default (0)
        ...
        try:
            ...
            # FIX 1: Set pyttsx3 rate back to default
...
def _speak_pyttsx3(text):
    ...
    try:
        ...
        # Delay to ensure audio starts before worker exits
        time.sleep(0.5) 
...
def _speak_win32com(text):
    ...
    if tts_engine_win:
        try:
            # SVSFlagsAsync = 1 (truly non-blocking)
            tts_engine_win.Speak(text, 1) 
            
            # --- CRITICAL FIX: Add a necessary delay after sending the command. ---
            # This ensures the short "How may I help?" phrase finishes playing 
            # before the TTS worker calls task_done() and unblocks the main thread's mic activation.
            time.sleep(2) 
...
# -------------------------------------------------------------
## Wake Word Engine Functions (No Change)
# -------------------------------------------------------------
...
# -------------------------------------------------------------
## Main Execution (Live STT) (No Change)
# -------------------------------------------------------------
...
def listen_for_command(recorder, recognizer):
    ...
    # This blocks until the TTS Worker calls task_done() after the audio delay
    speech_queue.join() 
...
# -------------------------------------------------------------
## PDF Reading Skill and Intent Parser (No Change)
# -------------------------------------------------------------

def command_read_pdf(file_name="data/sample.pdf"):
    """Read and speak the full PDF content."""
    file_path = Path(__file__).parent / file_name
    
    jarvis_speak(f"Parsing the document, {file_path.name}. Please wait.")
    
    clean_text = pdf_parse(str(file_path))
    
    if not clean_text:
        jarvis_speak("I could not find or read the document.")
        return False
    
    jarvis_speak(f"Now reading the full document. {clean_text}")
...
def parse_command_for_intent(command):
    ...
    # FIX 2: Ignore the command if the user simply repeats the confirmation prompt.
    if "how may i help" in command or "at your service" in command:
        print(f"LOGIC: Ignoring self-referential command: '{command}'")
        return True
    
    if any(phrase in command for phrase in ["read pdf", "read document", "analyze pdf"]):
        command_read_pdf()
        return True
    else:
        # This is now the ONLY path to the "I am sorry" response
