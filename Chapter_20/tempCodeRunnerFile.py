...
# -------------------------------------------------------------
## 7. NEW: Intelligent Command Processor (Section 2.4)
# -------------------------------------------------------------

def parse_command_for_intent(command, voice_name):
    """Uses TextBlob to analyze the command and route the request."""
    
    # 1. Create the TextBlob Object
    blob = TextBlob(command)
    
    # 2. Extract Noun Phrases (The 'Who/What')
    # Convert phrases to a list of lower-case strings for easy matching
    key_phrases = [str(p).lower() for p in blob.noun_phrases]
    
    # 3. Check Sentiment (The 'Tone')
    sentiment_score = blob.sentiment.polarity
    
    # --- Intent Routing Logic ---
    
    # First: Check for strongly negative sentiment and respond contextually
    if sentiment_score < -0.3:
        jarvis_speak("I apologize for any issue. I will try to find a better result for you.", voice_name)
        # Continue to execute the command if a target is found
    
    # Second: Intent 1 - YouTube Search (Check for 'video', 'watch', 'youtube' keywords)
    if any(k in command.lower() for k in ["video", "youtube", "watch"]):
        # Use the key phrases as the search query
        search_query = " ".join(key_phrases)
        if search_query:
            # We don't have a YouTube-specific function yet, so we use the default web search 
            # but announce the intent. (The next chapter will make this a YouTube-specific call)
            jarvis_speak(f"Routing to YouTube. Searching for: {search_query}.", voice_name)
            execute_web_search(f"youtube {search_query}", voice_name) 
            return
        
    # Third: Intent 2 - General Web Search (Default action if key phrases were found)
        ...
        # Fallback if TextBlob couldn't find a subject and no specific keyword was used
        jarvis_speak("Command acknowledged, but no specific action or target found.", voice_name)
